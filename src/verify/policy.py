...
def classify(
    ...
    if support >= thresholds["T_support_ok"] and contra < thresholds["T_contra_veto"]:
        ...
        # Covers support=0 (no supporting evidence) regardless of contra weight.
        # A claim contradicted but not supported is still "unsupported" — the finalizer
        # drops it, which is safer than "contested" (which would present both sides).
        verdict = "unsupported"
